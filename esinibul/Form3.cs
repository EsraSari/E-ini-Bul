    ...
    public partial class Form3 : Form
    {
        ...
        void Doldur()
        {
            try
            {
                ...
                SqlCommand sqlCommand = new SqlCommand("SELECT ROW_NUMBER() OVER(ORDER BY Seviye) SiraNo,* from SoruCevap where Seviye=2", cnn);
                ...
                if (reader.HasRows)
                {
                    ...
                    //DataRow dRow = (from r in dt.Select().AsEnumerable() where !r.IsNull("SiraNo") && r.Field<int>("SiraNo") == sayilar[soruno] select r).FirstOrDefault();
                    //if (dRow != null)
                    //{
                    //    button1.Text = dRow["Soru"].ToString();
                    //}
